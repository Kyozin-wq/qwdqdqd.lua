...
local function trigger(player)
    ...
    -- 🔒 Auto Block só dentro do range
    if Config.AutoBlockEnabled and inRange(player) then
        RMB()
    end

    -- 🔒 Auto Clash só dentro do range
...
local function createGUI()
    ...
    frame.Position = UDim2.new(0.7,0,0.5,-110)
    ...
    title.Text = "Highlight + Auto Clash + Auto Block"
    ...
    local L1 = makeLabel(40)
    local L2 = makeLabel(70)
    local L3 = makeLabel(100)

    local function makeButton(text, y, action)
        local b = Instance.new("TextButton", frame)
        b.Size = UDim2.new(0.3, -10, 0, 25)
        b.Position = UDim2.new(0.68,5,0,y)
        ...
        b.MouseButton1Click:Connect(function()
            ...
            b.Text = "Pressione a tecla..."
        end)
        return b
    end

    local B1 = makeButton("Rebind", 40, "Highlight")
    local B2 = makeButton("Rebind", 70, "AutoClash")
    local B3 = makeButton("Rebind", 100, "AutoBlock")

    -- RGB rápido e vivo
    local hue = 0
    spawn(function()
        while true do
            hue = (hue + 10) % 360 -- efeito super rápido
            local function HSV(h,s,v)
                return Color3.fromHSV(h/360, s, v)
            end

            L1.TextColor3 = Config.HighlightEnabled and HSV(hue,1,1) or Color3.fromRGB(128,128,128)
            L2.TextColor3 = Config.AutoClashEnabled and HSV(hue,1,1) or Color3.fromRGB(128,128,128)
            L3.TextColor3 = Config.AutoBlockEnabled and HSV(hue,1,1) or Color3.fromRGB(128,128,128)
    ...
    local function refresh()
        ...
        B1.Text = "Rebind ("..Config.Keybinds.Highlight.Name..")"
        B2.Text = "Rebind ("..Config.Keybinds.AutoClash.Name..")"
        B3.Text = "Rebind ("..Config.Keybinds.AutoBlock.Name..")"
    end
